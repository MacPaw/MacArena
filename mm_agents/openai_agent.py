    ...
    def _create_response(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        while retry_count < MAX_RETRIES:
            try:
                ...
                last_message = self.history_responses[-1]
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        if self.history_responses == []:
            ...
            self.history_responses.append({
                ...
                "call_id": self.last_action["call_id"],
                "acknowledged_safety_checks": self.last_action["pending_checks"],
                "output": {
                    "type": "input_image",
                    "image_url": f"data:image/png;base64,{base64_image}",
                },
        ...
        logger.info(f"Model response time: {model_timer.duration:.2f}s")

        self.history_responses += response.output
        ...
        predict_info = {
            ...
            "messages": self.history_responses,
            "response": "\n".join(responses) if isinstance(responses, list) and all(isinstance(item, str) for item in responses) else "",
            "state_correct": state_correct,
        }
        
        self.last_action = actions[-1] if len(actions) > 0 else {"action_space": "pyautogui", "action": "DONE", "pending_checks": [], "call_id": ""}
        if not actions:
            # add done action if no action is predicted
