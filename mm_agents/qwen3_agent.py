...
{previous_actions_str}"""
        ...
        history_len = min(self.history_n, len(self.history_responses))
        if history_len > 0:
            history_history_responses = self.history_responses[-history_len:]
                ...
                messages.append(
                    {
                        "role": "assistant",
                        "content": [
                            {"type": "text", "text": f"{history_history_responses[idx]}"},
                        ],
                    }
                )
