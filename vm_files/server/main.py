...
def launch_app():
    ...
    try:
        ...
        if 'chromium' in command and platform_name == "Darwin":
            index = command.index('chromium')
            command[index] = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'

            command.extend([f"--user-data-dir={os.path.expanduser('~/user-data-dir')}"])
...
def get_terminal_output():
    ...
    try:
        if user_platform == "Linux":
            ...
            name = "Terminal"
            bundle_id = macapptree.get_app_bundle(name)
            tree = macapptree.get_tree(bundle_id)
            ...
            while stack:
                ...
                if role == "AXTextArea" and description == "shell" and role_description == "text entry area":
                    output = element["value"].rstrip()
                    break
...
@app.route("/macapptree_accessibility", methods=["GET"])
def get_macapptree_accessibility_tree():
    app_name = request.args.get("app_name")
    if not app_name:
        return jsonify({"error": "app_name is required"}), 400

    bundle_id = macapptree.get_app_bundle(app_name)
    if not bundle_id:
        return jsonify({"error": "app_name is not a valid app"}), 400

    at = macapptree.get_tree(bundle_id)
...
def open_file():
    ...
    try:
        ...
        ## TODO: implement opening of file/applicaiton on macOS
        ...
        return "Opened file/application successfully"
    except Exception as e:
...
def activate_window():
    ...
    if os_name == 'Windows':
        ...
        subprocess.Popen(["open", "-a", window_name])
...
def close_window():
    ...
    if os_name == "Windows":
        ...
        script = f'''quit app "{window_name}"'''
        process = subprocess.Popen(['osascript', '-e', script], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, error = process.communicate()
        if error:
            app.logger.error("Error: %s", error.decode('utf-8'))
            return f"Failed to close window {window_name}. Error: {error.decode('utf-8')}", 500
    else:
...
def start_recording():
    ...
    if platform_name == "Darwin":
        screen_width = int(AppKit.NSScreen.mainScreen().frame().size.width)
        screen_height = int(AppKit.NSScreen.mainScreen().frame().size.height)

        start_command = f"""ffmpeg -y \
-f avfoundation -framerate 30 \
-video_size {screen_width}x{screen_height} -capture_cursor 1 \
-i "0:none" \
