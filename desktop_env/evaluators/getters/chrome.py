...
def get_default_search_engine(env, config: Dict[str, str]):
    ...
    if os_type == 'Windows':
        ...
        preference_file_path = env.controller.execute_python_command(
            "import os; print(os.path.join(os.getenv('HOME'), 'user-data-dir/Default/Preferences'))")[
            'output'].strip()
    elif os_type == 'Linux':
    ...
    try:
        ...
        search_engine = data.get('default_search_provider_data', {})\
            .get('mirrored_template_url_data', {})\
...
def get_history(env, config: Dict[str, str]):
    ...
    if os_type == 'Windows':
        ...
        chrome_history_path = env.controller.execute_python_command(
            """import os; print(os.path.join(os.getenv('HOME'), "user-data-dir", "Default", "History"))""")[
            'output'].strip()
    elif os_type == 'Linux':
        if "arm" in platform.machine():
            chrome_history_path = env.controller.execute_python_command(
...
def get_bookmarks(env, config: Dict[str, str]):
    ...
    if os_type == 'Windows':
        ...
        preference_file_path = env.controller.execute_python_command(
            "import os; print(os.path.join(os.getenv('HOME'), 'user-data-dir/Default/Bookmarks'))")[
            'output'].strip()
    elif os_type == 'Linux':
        if "arm" in platform.machine():
            preference_file_path = env.controller.execute_python_command(
...
def get_active_url_from_accessTree(env, config):
    ...
    try:
        if platform.system() == "Darwin":
            accessibility_tree = accessibility_tree.replace(" st:", " ") \
                .replace(" attr:", " ") \
                .replace(" cp:", " ") \
                .replace(" doc:", " ") \
                .replace(" txt:", " ") \
    ...
    if platform.system() == "Darwin":
        selector_string = 'standard_window[name="Google Chrome"] text_field[AXDescription="Address and search bar"]'
    elif "arm" in arch:
...
def get_shortcuts_on_desktop(env, config: Dict[str, str]):
    ...
    if os_name == 'Windows':
        ...
        # macOS's shortcuts are .crwebloc files
        shortcut_extension = '.crwebloc'
    elif os_name == 'Linux':
