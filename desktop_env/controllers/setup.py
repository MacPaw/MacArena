...
class SetupController:
    ...
    def setup(self, config: List[Dict[str, Any]], use_proxy: bool = False)-> bool:
        ...
        while retry < MAX_RETRIES:
            try:
                _ = requests.get(self.http_server + "/screenshot")
                break
            except:
                time.sleep(5)
                retry += 1
    ...
    def _update_browse_history_setup(self, **config):
        ...
        if os_type == "Darwin":
            self._execute_setup(["sudo chown -R admin:admin ~/user-data-dir/Default/History"], shell=True)
        else:
            self._execute_setup(["sudo chown -R admin:admin ~/.config/google-chrome/Default/History"], shell=True)
