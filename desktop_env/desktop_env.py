...
class DesktopEnv(gym.Env):
    ...
    def __init__(
        ...
        if client_password == "":
            ...
            elif self.provider_name in {"utm"}:
                self.client_password = "admin"
            else:
        ...
        if self.provider_name in {"docker", "aws", "gcp", "azure", "aliyun", "volcengine", "utm"}:
    ...
    def _set_task_info(self, task_config: Dict[str, Any]):
        ...
        self.pre_upload_files = task_config.get("pre_upload_files", [])
        self._set_evaluator_info(task_config)

        self.config = []

        if self.pre_upload_files:
            self.config.append({
                "type": "upload_file",
                "parameters": {
                    "files": self.pre_upload_files
                }
            })

        if "pre_command" in task_config and task_config["pre_command"]:
            self.config.append({
                "type": "launch",
                "parameters": {
                    "command": task_config["pre_command"],
                    "shell": True
                },
            })

        if "config" in task_config and task_config["config"]:
            self.config.extend(task_config["config"])
    ...
    def _set_evaluator_info(self, task_config: Dict[str, Any]):
        ...
        if self._get_evaluator_type() == "osworld":
    ...
    def step(self, action, pause=2):
        ...
        action_type = action if isinstance(action, str) else action.get('action_type', None)
        ...
        if self.action_space == "computer_13":
            ...
            if action_type in ['WAIT', 'FAIL', 'DONE']:
                self.controller.execute_action(action_type)
            else:
    ...
    def _osworld_evaluate(self) -> float:
    ...
    def _macosworld_evaluate(self) -> float:
        self.is_environment_used = True
        filtered_eval_configs = [item for item in self.evaluator if item[1] == 100]
        for eval_config in filtered_eval_configs:
            command, return_value = eval_config
            # response = self.controller.execute_command(command, shell=True)
            print(command)
            response = self.controller.run_bash_script(command)
            print(response)

            status = response.get("status", "")
            output = response.get("output", "")
            if status == "success" and isinstance(output, str):
                if "true" in output.strip().lower():
    ...
    def evaluate(self):
        ...
        """
        evaluator_type = self._get_evaluator_type()

        if evaluator_type == "osworld":
            return self._osworld_evaluate()
        else:
            return self._macosworld_evaluate()
